...
from utils import set_seed, get_logger, get_model
from utils.ddp import reduce_mean, ProgressMeter, accuracy, AverageMeter

from configs import AllConfig
from omegaconf import DictConfig
from utils import  OutputHook, sethook
from loss import TET_loss
...
def train(train_loader, model, criterion, base_metrics, optimizer, epoch, local_rank, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if args.add_time_dim:
            images = images.unsqueeze(1)
            images = images.repeat(1,args.T,1,1,1)
        ...
        if regularization:
            ...
            _target = torch.unsqueeze(target,dim=0) 
            index = -int(outputs.shape[0]*args.rcs_n)
            right_predict_mask = outputs[index:].max(-1)[1].eq(_target).to(torch.float32)
            ...
            cs_mean = cs_mean*right_predict_mask
            cs_mean = cs_mean.max(dim=0)[0]
            cs_loss = cs_mean.mean()

        mean_out = outputs.mean(0)
        if args.loss == 'tet':
            loss = TET_loss(outputs, target, criterion, args.means, args.lamb)
        else:
